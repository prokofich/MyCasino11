...
class MiddleAdapter(private val context: Context, private val intrfc:InterfaceLevelUp): RecyclerView.Adapter<MiddleAdapter.MiddleViewHolder>() {
    ...
    private val clickDelay = 10000L // Задержка в миллисекундах (7 секунд)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int):MiddleViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_rv_number_middle,parent,false)
        return MiddleViewHolder(view)
    }

    override fun onBindViewHolder(holder:MiddleViewHolder, position: Int) {
        job = CoroutineScope(Dispatchers.Main).launch {
            holder.itemView.id_item_rv_middle_img.text = listNumber[position].toString()
            delay(10000)
            holder.itemView.id_item_rv_middle_img.text = ""
        }
    }
        ...
        holder.itemView.setOnClickListener {
            ...
            if (currentTime - fragmentStartTime >= clickDelay && listNumber[holder.adapterPosition] !in listMyAnswers) {
                ...
                job2 = CoroutineScope(Dispatchers.Main).launch {
                    holder.itemView.id_item_rv_middle_img.isEnabled = false
                    if(listMyAnswers.size==0){
                        if(listNumber[holder.adapterPosition].toString() == "1"){
                            holder.itemView.id_item_rv_middle_img.text = listNumber[holder.adapterPosition].toString()
                            ...
                            showToast("wrong...")
                            delay(2000)
                            GameFragment.gameOver(COMPLEXITY_MIDDLE,currentLevel)
                        }
                    }else{
                        if(listNumber[holder.adapterPosition] == listMyAnswers[listMyAnswers.size-1]+1){
                            if(listMyAnswers.size==15){
                                holder.itemView.id_item_rv_middle_img.text = listNumber[holder.adapterPosition].toString()
                                listMyAnswers.add(listNumber[holder.adapterPosition])
                                showToast("VICTORY!")
                                delay(2000)
